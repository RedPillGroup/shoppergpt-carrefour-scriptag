...
export function MenuBuilderPanel({ requirements, productsByStep, quantities, onQuantityChange }: Props) {
  ...
  // Steps that have ≥ 1 product with qty > 0
  const stepsWithItems = useMemo(() => {
    const set = new Set<string>();
    for (const [id, qty] of Object.entries(quantities)) {
      if (qty <= 0) continue;
      for (const [step, products] of Object.entries(productsByStep)) {
        if (products.some(p => p.id === id)) { set.add(step); break; }
      }
    }
    return set;
  }, [quantities, productsByStep]);
  ...
  return (
    <div class="flex-1 min-h-0 flex flex-col overflow-hidden bg-[#FAF9F7]">
      ...
      <div class="shrink-0 bg-white border-b border-[#E8ECF0] px-4 py-3 md:px-5 flex items-center gap-3">
        {/* Cursive event info — centred, takes remaining space */}
        <div class="flex-1 flex flex-col items-center gap-1.5 min-w-0">
          <div class="border border-[#D9C9A8] rounded-full px-4 py-1 max-w-full">
            <span class="font-['Satisfy'] text-[#C7B287] text-[15px] md:text-base capitalize leading-none truncate block text-center">
              {eventLabel}
            </span>
          </div>
          {dateLabel && (
            <div class="border border-[#E8D9C0] rounded-full px-3 py-0.5 bg-[#FDFAF6]">
              <span class="font-['Satisfy'] text-[11px] md:text-[12px] text-[#B09A6E] leading-none">
                {dateLabel}
              </span>
            </div>
          )}
        ...
        <button
          class="shrink-0 flex flex-col items-center gap-1 text-[#9A8C78] hover:text-[#C7B287] transition-colors border border-[#E8D9C0] rounded-xl px-2.5 py-2 bg-white"
        ...
        >
          ...
          <span class="text-[7px] md:text-[8px] uppercase tracking-wide font-semibold leading-none text-center">
            Liste
          </span>
        </button>
      </div>
            ...
            <div class="flex flex-col gap-8">
              {steps.map(step => {
                ...
                return (
                  <section
                    key={step}
                    ref={(el) => { sectionRefs.current[step] = el as HTMLElement | null; }}
                  >
        ...
        <div class="shrink-0 bg-white border-t border-[#E8ECF0]">
          <div class="flex items-stretch overflow-x-auto [&::-webkit-scrollbar]:hidden">
            {steps.map(step => {
              ...
              if (!icon) return null;
              const hasItems = stepsWithItems.has(step);
              const hasProductsForStep = (productsByStep[step]?.length ?? 0) > 0;
              return (
                <button
                  key={step}
                  onClick={() => scrollToStep(step)}
                  class={`relative flex-1 min-w-[52px] flex flex-col items-center gap-1 py-2.5 px-1 border-0 cursor-pointer transition-colors duration-150 ${
                    hasProductsForStep
                ...
                >
                  {hasItems && (
                    <div class="absolute top-1 left-1/2 -translate-x-3 w-3.5 h-3.5 bg-[#16a34a] rounded-full flex items-center justify-center">
                      <svg width="7" height="7" viewBox="0 0 7 7" fill="none">
                        <path d="M1 3.5l1.8 1.8L6 1.5" stroke="white" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </div>
                  )}
                  <span class="h-[18px] w-[18px] flex items-center justify-center shrink-0 mt-1">{icon}</span>
      ...
      <div class="shrink-0 grid grid-cols-2 border-t border-[#E8ECF0] shadow-[0_-4px_14px_rgba(17,24,39,0.06)]">
        <div class="bg-[#F3F1EE] px-3 py-2.5 md:px-4 md:py-3 flex flex-col gap-1.5">
          <div class="flex items-baseline justify-between gap-1">
            <span class="text-[9px] md:text-[10px] font-semibold uppercase tracking-wide text-[#8A8070] shrink-0">Convives</span>
          ...
          <div class="flex items-baseline justify-between gap-1">
            <span class="text-[9px] md:text-[10px] font-semibold uppercase tracking-wide text-[#8A8070] shrink-0">Prix/pers.</span>
        ...
        <div class="bg-[#C7B287] text-white px-3 py-2.5 md:px-4 md:py-3 flex flex-col gap-1.5">
          <div class="flex items-baseline justify-between gap-1">
            <span class="text-[9px] md:text-[10px] font-semibold uppercase tracking-wide text-[#F7F2E6] shrink-0">Budget</span>
          ...
          <div class="flex items-baseline justify-between gap-1">
            <span class="text-[9px] md:text-[10px] font-semibold uppercase tracking-wide text-[#F7F2E6] shrink-0">Coût total</span>
            <span class="text-[9px] md:text-[10px] text-white tabular-nums font-semibold">
